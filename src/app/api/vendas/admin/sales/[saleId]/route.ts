...
import { deleteSalesSale, loadSalesDetail, salesDeleteInputSchema, salesUpsertInputSchema, upsertSalesSale } from "@/lib/sales-admin-sales";
...
export async function DELETE(request: Request, { params }: { params: { saleId: string } }) {
  if (!UUID.test(params.saleId)) return NextResponse.json({ error: { code: "VALIDATION", message: "Venda inválida." } }, { status: 400 });
  const session = await requireSalesAdminSession(); if (session.error) return session.error;
  const payload = await parseSalesAdminBody(request, salesDeleteInputSchema); if (payload.error) return payload.error;
  return salesAdminResultResponse(await deleteSalesSale(session.supabase, params.saleId, payload.data));
}
