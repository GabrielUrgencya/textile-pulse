...
export async function POST(request: Request) {
  ...
  const { data: active } = await supabaseAdmin
    .from("faction_tokens")
    .select("id, token, pin_plain, created_at")
    .eq("tenant_id", tenantId)
  ...
  if (!canonical) {
    ...
    const { data: created, error } = await supabaseAdmin
      .from("faction_tokens")
      .insert({ tenant_id: tenantId, faction_id: factionId, name: faction.name, pin_hash: pinHash, pin_plain: pin })
      .select("id, token, pin_plain")
      .single();
    if (error || !created) {
      return NextResponse.json({ error: "Falha ao criar o acesso" }, { status: 500 });
    }
    canonical = { ...created, created_at: new Date().toISOString() };
  } else if (action === "rotate" || !canonical.pin_plain) {
    ...
    const { error } = await supabaseAdmin
      .from("faction_tokens")
      .update({ pin_hash: pinHash, pin_plain: pin })
      .eq("id", canonical.id);
    if (error) {
      return NextResponse.json({ error: "Falha ao gerar novo PIN" }, { status: 500 });
    }
  } else {
    // "get" idempotente: o acesso atual já existe e tem PIN — devolve como está.
